...
try:
    ...
    print("MongoDB Atlas connection failed:", err)
...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        if 'file_password' in user:
            ...
            flash('Please set a file encryption password first.')
            return redirect(url_for('set_file_password'))
        ...
        if file:
            ...
            flash('File secured and uploaded successfully!', 'success')
            # MOVE TO DASHBOARD: Change 'index' to 'home' if that's your dashboard route
            return redirect(url_for('home')) 
...
def notifications():
    ...
    notifications = list(db.access_requests.find({'sender': session['username']}))
    print("Notifications:", notifications)  # debug
